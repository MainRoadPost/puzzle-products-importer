...
class ProductTreeNode:
    ...
    def __init__(
        self,
        name: str,
        kind: ProductKind = ProductKind.GROUP,
        product_data: CsvRow | None = None,
    ):
        """
        ...
        self.children: dict[str, ProductTreeNode] = {}
...
class PuzzleUploaderUI(QWidget):
    ...
    async def get_domains(self):
        ...
        try:
            ...
            self.domain_combo.addItem("No domain", None)
            for domain in response.domains:
                self.domain_combo.addItem(domain.name, domain.name)
    ...
    async def attempt_login(self):
        ...
        self.login_status_label.setText(
            f"Attempting login to {self.domain_combo.currentText()}..."
        )
    ...
    def parse_csv_file(self, file_path: str):
    ...
    async def generate_mutation_queries(
        self, root: ProductTreeNode, parent_id: str | None = None
    ):
        ...
        for child_name, child_node in root.children.items():
            self.import_status_label.setText(
                f"Importing {child_node.kind.lower()} '{child_name}'..."
            )
            ...
            if existing_item:
                if existing_item["kind"] != child_node.kind:
                    logging.warning(
                        f"Conflict: '{child_name}' is a {existing_item['kind']} but expected {child_node.kind}."
                    )
                    continue
                if self.update_mode and child_node.kind == "PRODUCT":
                    ...
                    logging.info(
                        f"{child_name} already exists as {child_node.kind}. Skipping creation."
                    )
    ...
    async def create_product_group(self, node: ProductTreeNode, parent_id: str | None):
    ...
    async def create_product(self, node: ProductTreeNode, parent_id: str | None):
        ...
        if product_data is None:
            logging.warning(f"No product data for {node.name}, skipping creation.")
            return
        ...
        if status_str == "" or status_str == "ACTIVE":
            status_value = ProductStatusEnum.ACTIVE
        elif status_str == "COMPLETED":
            status_value = ProductStatusEnum.COMPLETED
        elif status_str == "CANCELED":
            status_value = ProductStatusEnum.CANCELED
        else:
            logging.warning(
                f"Invalid status '{status_str}' for product '{node.name}'. Using default 'ACTIVE'."
            )
            status_value = ProductStatusEnum.ACTIVE
        ...
        picture_field = product_data.picture.strip()
        image_path = None
        if picture_field:
            image_path = os.path.join(
                os.path.dirname(self.csv_file_path), picture_field
            )
